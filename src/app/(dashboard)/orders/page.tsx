...
// ─── Mock Data ────────────────────────────────────────────────────────────────
const mockRecentOrders: Order[] = [
  { id: 'ORD-073', customer: 'John Smith',    email: 'john.smith@example.com', status: 'Fulfilled',           date: '2024-10-28', amount: 12500, currency: 'INR', source: 'Web',    pnr: 'L8Y2N3', route: 'BOM-DXB', paymentStatus: 'Paid'     },
  { id: 'ORD-072', customer: 'Sarah Chen',    email: 's.chen@corporate.com',   status: 'Fulfilled',           date: '2024-10-28', amount: 8400,  currency: 'INR', source: 'CUTE',   pnr: 'P4X5T6', route: 'DEL-SIN', paymentStatus: 'Paid'     },
  { id: 'ORD-071', customer: 'Michael Smith', email: 'm.smith@voyage.co',      status: 'Pending',             date: '2024-10-27', amount: 450,   currency: 'INR', source: 'CUSS',   pnr: 'GRP923', route: 'BOM-DXB', paymentStatus: 'Pending'  },
  { id: 'ORD-070', customer: 'Alice Johnson', email: 'alice.j@web.com',        status: 'Canceled',            date: '2024-10-27', amount: 120,   currency: 'INR', source: 'Mobile', pnr: 'M9Z1M2', route: 'DEL-SIN', paymentStatus: 'Refunded' },
  { id: 'ORD-069', customer: 'Robert Brown',  email: 'robert.b@travel.com',    status: 'Partially Fulfilled', date: '2024-10-26', amount: 75,    currency: 'INR', source: 'Web',    pnr: 'K7L9P2', route: 'DEL-SIN', paymentStatus: 'Paid', isSimulated: true },
];
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          ...
          {row.original.isSimulated && (
            <Badge variant="outline" className="text-[8px] h-3.5 bg-indigo-50 border-indigo-200 text-indigo-700 font-black">
              SIMULATED
            </Badge>
          )}
        </div>
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2">
        <Badge variant="secondary" className="font-mono text-[10px]">
          {row.original.route || 'N/A'}
        </Badge>
        {row.original.airportCode && (
          <span className="text-[10px] text-muted-foreground">@{row.original.airportCode}</span>
        )}
      </div>
    ),
  },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col items-end gap-1">
        ...
        <Badge
          variant={getPaymentStatusBadgeVariant(row.original.paymentStatus)}
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          <DropdownMenuItem asChild>
            <Link href={`/orders/${row.original.id}`}>
              <ReceiptText className="mr-2 h-4 w-4" /> View Full Details
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href={`/orders/${row.original.id}/lineage`}>
              <ArrowRightLeft className="mr-2 h-4 w-4" /> Trace Lineage
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <Link href={`/orders/servicing?orderId=${row.original.id}`}>
              <Settings className="mr-2 h-4 w-4" /> Modify / Reshop
            </Link>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          ...
          {row.original.pnr && (
            <DropdownMenuItem onClick={() => navigator.clipboard.writeText(row.original.pnr!)}>
              Copy PNR
            </DropdownMenuItem>
          )}
        </DropdownMenuContent>
...
// ─── Page Component ───────────────────────────────────────────────────────────
export default function OrdersPage() {
  // ── 1. Firebase setup with null guard ──────────────────────────────────────
  const firestore = useFirestore();
  ...
  const { data: liveOrders, loading: firestoreLoading } = useCollection(ordersQuery);

  // ── 2. Timeout guard — prevents infinite loading freeze ───────────────────
  const [timedOut, setTimedOut] = React.useState(false);

  React.useEffect(() => {
    if (!firestoreLoading) {
      setTimedOut(false); // reset if it loads fast next time
      return;
    }
    const timer = setTimeout(() => {
      setTimedOut(true); // after 5s, stop waiting → fall back to mock data
    }, FIREBASE_TIMEOUT_MS);

    return () => clearTimeout(timer);
  }, [firestoreLoading]);

  // ── 3. Derived loading state — true only if loading AND not yet timed out ─
  const loading = firestoreLoading && !timedOut;

  // ── 4. Data resolution — mock data fallback ───────────────────────────────
  const data = React.useMemo<Order[]>(() => {
    // While genuinely loading (not timed out), return empty so spinner shows
    if (loading) return [];

    // If Firebase returned real orders, use them
    if (liveOrders && liveOrders.length > 0) return liveOrders as Order[];

    // Otherwise (timeout, empty collection, no firebase) → use mock data
  ...
  const displayOrders = React.useMemo(() => {
    ...
    if (activeFilters.status && activeFilters.status !== 'All')
      result = result.filter((o) => o.status === activeFilters.status);

    if (activeFilters.source && activeFilters.source !== 'All')
      result = result.filter((o) => o.source === activeFilters.source);

    if (activeFilters.paymentStatus && activeFilters.paymentStatus !== 'All')
      result = result.filter((o) => o.paymentStatus === activeFilters.paymentStatus);
  ...
  return (
    <div className="flex flex-col gap-6 min-h-screen">
      ...
      <TableFilterBar
        ...
        onRemoveChip={(k) => removeFilter(k as keyof typeof activeFilters)}
        onClearAll={clearAll}
      />
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base font-bold text-gray-900">
                Retailing Conversion Queue
              </CardTitle>
              <CardDescription className="mt-0.5 text-xs text-gray-400">
                Live real-time trace of ecosystem transactions across Web, Mobile, and SITA touchpoints.
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" asChild>
                <Link href="/orders/delivery">
                  <Truck className="mr-2 h-4 w-4" /> Fulfillment Queue
                </Link>
              </Button>
              <Button asChild>
                <Link href="/offer-composer">
                  <MonitorDot className="mr-2 h-4 w-4" /> Simulated Touchpoint
                </Link>
              </Button>
            </div>
          </div>
        ...
        <CardContent className="pt-2">
          <div className="rounded-xl border border-gray-200 overflow-hidden">
            <Table>
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  ...
                  >
                    {headerGroup.headers.map((header) => (
                      <TableHead
                        key={header.id}
                        className="py-3 text-[11px] font-bold uppercase tracking-wider text-gray-500"
                      >
                        {flexRender(header.column.columnDef.header, header.getContext())}
                      </TableHead>
                    ))}
              ...
              <TableBody>
                {/* ── Loading spinner (only while genuinely loading) ── */}
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={columns.length} className="h-64 text-center">
                      <div className="flex flex-col items-center gap-2">
                        <Loader2 className="h-8 w-8 animate-spin text-violet-600/40" />
                        <p className="text-xs text-gray-400">Streaming conversion data...</p>
                      </div>
                    </TableCell>
                  </TableRow>
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      key={row.id}
                      className="transition-colors duration-100 hover:bg-violet-50/60 cursor-default"
                    >
                      {row.getVisibleCells().map((cell) => (
                  ...
                  <TableRow>
                    <TableCell colSpan={columns.length} className="h-64 text-center py-20">
