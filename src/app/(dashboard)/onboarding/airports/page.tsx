...
//                     {loadingAirports && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
...
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { PlusCircle, Search, MonitorDot, MoreHorizontal, Loader2, Plane, Check, Globe, CheckCircle2, RefreshCw, AlertCircle } from 'lucide-react';
import { Input } from '@/components/ui/input';
import {
  DropdownMenu,
  DropdownMenuContent,
...
const SITA_OPTIONS    = ["CUSS/CUTE", "CUSS Only", "CUTE Only", "None"].map((v) => ({ label: v, value: v }));

// ─── Sita Badge Component ─────────────────────────────────────────────────────
const SitaBadge = ({ sita }: { sita: string }) => (
  <span className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-violet-600 text-white text-xs font-semibold">
...
export default function AirportOnboardingPage() {
    ...
    const sourceData = useMemo(() => {
        return (airportsCollection && airportsCollection.length > 0) 
            ? airportsCollection as AirportOnboarding[] 
            : mockAirports;
    }, [airportsCollection]);
    ...
    const {
        ...
        searchFields: ["name", "iataCode", "location"],
        filterFields: { status: "", location: "", sitaEnabled: "" },
    });

    // Convert sitaEnabled boolean to string for filtering
    const getSitaString = (sitaEnabled: boolean) => {
        return sitaEnabled ? "CUSS/CUTE" : "None";
    };
    ...
    return (
        <div className="flex flex-col gap-6 min-h-screen">
            ...
            <TableFilterBar
                ...
                searchPlaceholder="Search by name or IATA code..."
                dropdowns={[
                    {
                        key: "status",
                        label: "Status",
                    ...
                    },
                    {
                        key: "sitaEnabled",
                        label: "SITA Type",
                        options: SITA_OPTIONS,
                        value: activeFilters.sitaEnabled ?? "All",
                        onChange: (v) => setFilter("sitaEnabled", v),
                    },
                ]}
            ...
            <Card className="border-none bg-white shadow-[0_2px_6px_rgba(0,0,0,0.06),0_-2px_6px_rgba(0,0,0,0.04)]">
                <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-base font-bold text-gray-900">Participating Airports</CardTitle>
                            <CardDescription className="mt-0.5 text-xs text-gray-400">
                                View and manage airports enabled for Offersense retailing.
                            </CardDescription>
                        </div>

                        <div className="flex items-center gap-2">
                            {loadingAirports && <Loader2 className="h-4 w-4 animate-spin text-gray-400" />}
                            <button
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-b border-gray-200 bg-gray-100 hover:bg-gray-100">
                                <TableHead className="py-3 text-[11px] font-bold uppercase tracking-wider text-gray-500">Airport Name</TableHead>
                                <TableHead className="py-3 text-[11px] font-bold uppercase tracking-wider text-gray-500">Code</TableHead>
                                <TableHead className="py-3 text-[11px] font-bold uppercase tracking-wider text-gray-500">City</TableHead>
                                <TableHead className="py-3 text-[11px] font-bold uppercase tracking-wider text-gray-500">SITA Enabled</TableHead>
                        ...
                        <TableBody>
                            {filtered.map((airport) => (
                                <TableRow key={airport.id} className="transition-colors duration-100 hover:bg-violet-50/60">
                                    <TableCell className="py-3.5 font-medium text-gray-900">{airport.name}</TableCell>
                                    <TableCell className="py-3.5 font-mono font-bold text-primary">{airport.iataCode}</TableCell>
                                    <TableCell className="py-3.5 text-gray-700">{airport.location}</TableCell>
                                    <TableCell className="py-3.5">
                                        {airport.sitaEnabled ? (
                                            <SitaBadge sita="CUSS/CUTE" />
                                        ) : (
                                            <Badge variant="outline">Web Only</Badge>
                                        )}
                                    </TableCell>
                                    ...
                                    <TableCell className="py-3.5 text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-56 rounded-xl border border-gray-100 shadow-xl">
                                                <DropdownMenuLabel className="text-xs font-semibold text-gray-500">Ecosystem Node Actions</DropdownMenuLabel>
                                                <DropdownMenuItem 
                                                    className="cursor-pointer text-xs font-semibold text-gray-600 hover:bg-violet-50 hover:text-violet-700"
                                                    onClick={() => handleOpenAirlinesDialog(airport)}
                                                >
                                                    <Plane className="mr-2 h-3.5 w-3.5 text-primary" /> Manage Enabled Airlines
                                                </DropdownMenuItem>
                                                <DropdownMenuItem 
